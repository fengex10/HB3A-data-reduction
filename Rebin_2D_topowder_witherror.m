function [ X_axis, data_new, err_new] = Rebin_2D_topowder_witherror( X, Y, data, err, dX, Yrange )
...
err_new = zeros(1, length(X_new)-1);
for ii = 1:length(X_new)-1
    ...
    if isempty(id)
        ...
        err_new(ii) = sqrt(sum(sum(err(id).^2)))/numel(err(id));
    end
